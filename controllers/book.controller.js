    ...
    try {
        ...
        if (search) {
            const books = await db
                ...
                .from(booksTable)
                .where(
                    sql`to_tsvector('english', ${booksTable.title} || ' ' || ${booksTable.description}) @@ to_tsquery('english', ${search})`
                );

            return res.status(200).json(books);   // IMPORTANT: return
        }

        const books = await db.select().from(booksTable);
        return res.status(200).json(books);     // second return
